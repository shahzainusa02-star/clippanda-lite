...
$("file").onchange=async()=>{file=$("file").files?.[0]||null;clips=[];$("resultsCard").classList.add("hidden");if(url)URL.revokeObjectURL(url);if(!file)return;url=URL.createObjectURL(file);source.src=url;$("fileInfo").textContent=`${file.name} · reading…`;try{await meta();duration=source.duration;$("fileInfo").textContent=`${file.name} · ${(file.size/1048576).toFixed(1)} MB · ${fmt(duration)}`;$("startSec").value=0;$("endSec").value=duration.toFixed(1);$("startSec").max=duration;$("endSec").max=duration;$("rangeBox").classList.remove("hidden");$("settings").classList.remove("hidden");syncRange()}catch(e){$("fileInfo").textContent=e.message}};
function syncRange(){let s=Math.max(0,+$("startSec").value||0),e=Math.min(duration,+$("endSec").value||duration);if(e<s+.5)e=Math.min(duration,s+.5);if(s>e-.5)s=Math.max(0,e-.5);$("startSec").value=s.toFixed(1);$("endSec").value=e.toFixed(1);$("startRange").value=duration?s/duration*100:0;$("endRange").value=duration?e/duration*100:100;$("rangeText").textContent=`Analyze ${fmt(s)} – ${fmt(e)} (${fmt(e-s)})`};$("startRange").oninput=()=>{$("startSec").value=(duration*+$("startRange").value/100).toFixed(1);syncRange()};$("endRange").oninput=()=>{$("endSec").value=(duration*+$("endRange").value/100).toFixed(1);syncRange()};$("startSec").onchange=syncRange;$("endSec").onchange=syncRange;
